...
@Component({
  ...
  styleUrls: ['./sender-message-input.component.scss'],
  standalone: false,
})
export class SenderMessageInputComponent {
  ...
  async sendNewMessage() {
    if (this.formGroup.get('send_input')?.value.trim() !== '') {
      const receiver_id = (
        await firstValueFrom(this.messageFacade.receiverUserInfos$)
      )?.id;
      const sender_id = (await firstValueFrom(this.userFacade.currentUser$))
        ?.id;
      this.messageFacade.sendNewMessage(
        this.formGroup.get('send_input')?.value.trim(),
        receiver_id!,
        sender_id!
      );
